...
/**
 * @swagger
 * /action_history/search/{field}/{value}/{page}/{order}/{type}:
 *   get:
...
app.get(
  ...
  (req, res) => {
    ...
    if (type !== "hour" && type !== "day") {
      ...
      res
        .status(400)
        .json({ error: "Bad request. Invalid type parameter provided." });
      return;
    }

    if (field === "hour") {
      query = "SELECT * FROM action_history WHERE HOUR(updatedAt) = ?";
      orderBy = "ORDER BY HOUR(updatedAt)";
    } else if (field === "day") {
      const formattedDate = value.split("-").reverse().join("-");
      query = "SELECT * FROM action_history WHERE DATE(updatedAt) = ?";
      orderBy = "ORDER BY DATE(updatedAt)";
      values = [formattedDate];
    } else if (field === "deviceID") {
      query = "SELECT * FROM action_history WHERE deviceID = ?";
      orderBy = "ORDER BY updatedAt";
    } else if (field === "action") {
      query = "SELECT * FROM action_history WHERE action = ?";
      orderBy = "ORDER BY updatedAt";
    } else {
      console.error("Invalid search field:", field);
      res
    ...
    if (type === "hour") {
      orderBy += ` ${order}, HOUR(updatedAt) ${order}`;
    } else if (type === "day") {
      orderBy += ` ${order}, DATE(updatedAt) ${order}`;
    }
